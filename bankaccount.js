...
class bankAccount {
	...
	let countedCapital = 0;
        ...
        for(let i=0; i<accArray.length; i++){
            //if (typeof accArray[i[this.balance]] === 'number') {
                countedCapital = countedCapital + accArray[i].balance;
            //}
        }
	...
	let results = [];
        var accountArr = bankAccount.getAccountsAsArray();
        for (let i=0; i<accountArr.length; i++){
            results[i] = {accountNumber: accountArr[i].accountNumber, owner: accountArr[i].ownerName, balance: accountArr[i].balance};     
        }  
	return results;
    }
}
